...
2. **로그인 링크 받기 →**를 누르고 `gimd50236@gmail.com`으로 온 메일의 링크를 엽니다. 기기마다 처음 한 번만 하면 됩니다.
...
작품 정보는 Supabase 데이터베이스에, 이미지는 Supabase Storage에 저장됩니다. 게시하거나 삭제하면 새 빌드를 기다리지 않고 모든 기기에 바로 반영됩니다. 방문자는 로그인 없이 작품을 볼 수 있고, 등록·삭제는 관리자 이메일로 로그인한 경우에만 허용됩니다. GitHub 토큰을 만들거나 입력할 필요가 없습니다.
